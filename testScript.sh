...
docker run -d --rm -p 8080:8080 --name gig-java-api-container gig-java-api
printf "Docker container is running\n\n"

until [ "$(curl -w '%{response_code}' --no-keepalive -o /dev/null --connect-timeout 1 localhost:8080/heartbeatt)" == "200" ];
do echo --- sleeping for 1 second;
sleep 1;
done

printf "Web application has started! Begnning Test...\n\n"

printf "Adding Account '0000001' with a balance of €20000\n"
addResponse1=$(curl --header "Content-Type: application/json" --request POST --data '{"balance": "20000","accountNumber": "0000001"}' localhost:8080/api/account/addOrUpdate)

printf "\nAdding Account '0000002' with a balance of €10000\n"
addRespons2=$(curl --header "Content-Type: application/json" --request POST --data '{"balance": "10000","accountNumber": "0000002"}' localhost:8080/api/account/addOrUpdate)
